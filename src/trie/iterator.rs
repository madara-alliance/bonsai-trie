...
pub struct MerkleTreeIterator<'a, H: StarkHash, DB: BonsaiDatabase, ID: Id> {
    ...
    current_path: Path,
    cur_path_nodes_heights: Vec<(NodeId, usize)>,
    current_value: Option<NodeHandle>,
}
...
impl<'a, H: StarkHash, DB: BonsaiDatabase, ID: Id> MerkleTreeIterator<'a, H, DB, ID> {
    ...
    pub fn seek_to(&mut self, key: &BitSlice) -> Result<(), BonsaiStorageError<DB::DatabaseError>> {
        ...
        let shared_prefix_len = self
            ...
            .take_while(|(a, b)| *a == *b)
            .count();
        // self.cur_path.truncate_to_shared_prefix(&key);
        ...
        self.cur_path_nodes_heights.truncate(nodes_new_len + 1);
            ...
            match node {
                Node::Binary(binary_node) => {
                    ...
                    *self
                        .current_path
                        .last_mut()
                        .expect("current path can't be empty if ") = bool::from(next_direction);
                    Some(binary_node.get_child_mut(next_direction))
                }
                Node::Edge(edge_node) => Some(&mut edge_node.child),
            }
        } else {
        ...
        loop {
            ...
            match node {
                ...
                Node::Edge(edge_node) => {
                    self.current_path.extend_from_bitslice(&edge_node.path.0);
...
mod tests {
    use crate::{
        ...
        id::BasicId,
        trie::{iterator::MerkleTreeIterator, merkle_node::{NodeHandle, NodeId}},
    ...
    fn test_iterator_seek_to() {
        ...
        // jump to non existent node, deviating from edge
        ...
        assert_eq!(iter.current_value, Some(NodeHandle::InMemory(NodeId(1))));
        assert_eq!(iter.current_path.0, bits![u8, Msb0; 0]);
        println!("{iter:?}");
